...
public class AcontecimientoBD{
    ...
    public  java.sql.Time conversionTime(LocalTime hora)
    {
      ...
      public Acontecimiento consultaPorNombre(String nombre,boolean asistentes) throws Exception
    {
       ...
       ResultSet resultado = sentenciaPre.executeQuery();
        if (resultado.next())
        {
            acontecimiento = crearObjeto(resultado,asistentes);
        }
        else
    ...
    public  Acontecimiento crearObjeto(ResultSet resultado, boolean asistentes) throws Exception
    {
        ...
        // ArrayList con los asistentes ????
        if (asistentes)
        {
            a.setAsistentes(asisbd.getAsistentes(a.getNombre()));
        }
        return a;
    }
            ...
            while (resultado.next())
            {
              ...
              // ¿ Hay plazas libres???
                if (asisbd.getNumeroAsistentes(resultado.getString("nombre"))< resultado.getInt("aforo"))
                {
                    ...
                    lista.add(crearObjeto(resultado,false));  
                }
            }
            return lista;
       }
